...
def fetch_and_save_data(request):
    ...
    for item in data:
        ...
        if symbol not in symbols_to_save:
            continue  # Skip symbols not in the specified list
        ...
        if expiry_date:
            try:
                ...
                expiry_date = fetch_live_data_for_expiry(symbol)
        else:
            expiry_date = fetch_live_data_for_expiry(symbol)
        ...
        # Check if the expiry date is current or future
        if expiry_date >= datetime.now().date():
            SYMBOL.objects.update_or_create(
                symbol_token=item.get('token'),  # Use 'symbol_token' instead of 'token'
                defaults={
                    'SYMBOL_order': item.get('symbol'),
                    'SYMBOL': item.get('name'),
                    'expiry': expiry_date,
                    'strike': item.get('strike') if item.get('strike') else fetch_live_data_for_strike(symbol),
                    'lotsize': item.get('lotsize') if item.get('lotsize') else fetch_live_data_for_lotsize(symbol),
                    'instrumenttype': item.get('instrumenttype'),
                    'exch_seg': item.get('exch_seg'),
                    'tick_size': item.get('tick_size') if item.get('tick_size') else fetch_live_data_for_tick_size(symbol),
                }
            )
...
#    try:
        ...
        #    user_id = request.user.id  # Assuming user is logged in and has an ID
            ...
            for i in range(1, len(filtered_signals) + 1):
                ...
                print(g.max_quantity , quantity)
                if int(quantity) <= int(g.max_quantity):
                    print(g.max_quantity , quantity ,symbol,'yes')
                    ...
                    print(g.max_quantity , quantity , symbol,"no")
...
from django.shortcuts import get_object_or_404
#http://127.0.0.1:8000/handle-smartapi-redirect/
from django.shortcuts import get_object_or_404
...
def get_symbol_token(symbol):
    symbol_token = instrument_dict.get(symbol)
    print(symbol_token)
    if not symbol_token:
        logger.error(f"Symbol {symbol} does not exist in instrument_dict")
    return symbol_token
...
def trade_history(request):
    ...
    for trade in trades:
        if trade.trade_type in ['Long_Entry', 'Short_Entry']:
            ...
            if entry_trade:
                pnl = (trade.price - entry_trade.price) * trade.lot_size if trade.trade_type == 'Long_Exit' else (entry_trade.price - trade.price) * trade.lot_size
                cumulative_pnl += pnl
...
def client_trade_history(request):
    if 'clint_id' in request.session:
        ...
        return render(request, 'trade_history_client.html', {'trade_history': trade_history, 'total_cumulative_pnl': cumulative_pnl,'client_user': client_user,})
    else:
        return redirect('/')
...
def client_status(request):
    if 'clint_id' in request.session:
        ...
        try:
            ...
            product_types = {csq.client_id: csq.Product_Type for csq in client_symbol_qty}
...
# def get_candle_data(obj, exchange, tradingsymbol, interval, from_date, to_date):
#     try:
...
#                 candles.append({
#                     'x': datetime.strptime(candle[0], "%Y-%m-%dT%H:%M:%S"),
#                     'o': float(candle[1]),
#                     'h': float(candle[2]),
#                     'l': float(candle[3]),
#                     'c': float(candle[4])
...
#             return candles
#         else:
#             print(f"Error fetching candle data: {historical_data['message']}")
#             return None
#     except Exception as e:
#         print(f"Error fetching candle data: {e}")
#         return None

# def fetch_latest_symbols():
#     url = "https://margincalculator.angelbroking.com/OpenAPI_File/files/OpenAPIScripMaster.json"
#     response = requests.get(url)
#     if response.status_code == 200:
...
#         symbols = fetch_latest_symbols()
#         if symbols:
#             for symbol_data in symbols:
#                 if symbol_data['symbol'] == symbol:
#                     exchange = symbol_data['exchange']  # Get exchange
#                     tradingsymbol = symbol_data['symbol']
#                     break
#             else:
#                 return JsonResponse({'error': 'Symbol not found in symbol master'})
#         else:
#             return JsonResponse({'error': 'Failed to fetch symbol master'})
...
#         else:
#             response_data = {'error': 'Error fetching candle data'}
...
def send_message_to_all_clients(request):
    if request.method == "POST":
        message = request.POST.get('message')
        clients = ind_clientDT.objects.filter(telegram_chat_id__isnull=False)
        responses = []

        for client in clients:
            response = send_telegram_message(client.telegram_chat_id, message)
            responses.append({
                'client': client,
                'response': response
            })

        return render(request, 'message_results.html', {'responses': responses})
    else:
        return redirect('send_message_form')
...
def telegram_webhook(request):
    if request.method == "POST":
        ...
        if "message" in data:
            ...
            # Store or update chat_id and client info
            client, created = ind_clientDT.objects.update_or_create(
                telegram_chat_id=chat_id,
                defaults={
                    'clint_name_first': first_name,
                    'clint_name_last': last_name,
                    'telegram_number': message["from"]["id"]  # Assuming telegram_number is user's Telegram ID
                }
            )

            # Respond to user
            if text == "/start":
                welcome_message = "Welcome! Your chat ID has been stored."
